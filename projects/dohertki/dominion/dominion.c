...
void _adventurer(int z, int currentPlayer, struct gameState *ptr_state){
  ...
  while(drawnTreasure<2){
	  ...
	  if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
	    ...
	    ptr_state->handCount[currentPlayer]--; //this should just remove the top card (the most recently drawn one).
	    z++;
	  }
  }
  while(z-1>=0){
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      _adventurer(z,currentPlayer, state);
      return 0;
    ...
    case mine:
        x = _mine(currentPlayer,choice1,choice2, handPos, state);
      return x;
    ...
    case great_hall:
      _great_hall(currentPlayer,handPos, state);
      return 0;
