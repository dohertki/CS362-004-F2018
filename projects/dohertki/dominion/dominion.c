...
while(drawntreasure<2){
	...
	if ((cardDrawn = copper) || cardDrawn == silver || cardDrawn == gold)  //FIXME cardDrawn == copper
	  drawntreasure++;
	else{
...
int council_roomEffect(int currentPlayer, struct gameState *state, int handPos)
{
 ...
 //+4 Cards
	...
	{
	  //if ( i != currentPlayer )  //FIXME remove comment 
	  //  {
	      drawCard(i, state);
	  //  }
	}
...
int villageEffect(int currentPlayer, struct gameState *state, int handPos){
      ...
      //discard played card from hand                      //FIXME missing discard
      //discardCard(handPos, currentPlayer, state, 0);
